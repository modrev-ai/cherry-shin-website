...
## X — dropped, nothing to configure

Reading a user timeline is not available on X's free tier, and the paid route was priced at roughly
**$3/month** at a 6-hour cache TTL — pay-per-usage credits at $0.001 per Owned Read, not a
subscription. The working is on MRO-242.

**The decision was to drop the platform**, on the ground that a few dollars a month is not the
question: the account's audience is elsewhere, and a permanently sampled tile on a live brand site
was the one outcome worth avoiding. X is removed from the feed and the platform table rather than
disabled in place, so there is nothing here to configure and no credential to hold.
