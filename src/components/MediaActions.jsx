...
// Renders a count only when there is one to render. A genuine zero still shows
// as "0" - the distinction being drawn is between a real figure and no figure,
// not between a big number and a small one. Sample posts carry no counts at
// all, so their buttons come through bare.
function Count({ value }) {
    const text = formatCount(value)
    if (text === null) return null
    return <span className="action-count">{text}</span>
}
    ...
    return (
        <div className="media-actions" onClick={stop}>
            {typeof item.views === 'number' && item.views > 0 && (
                <div className="action-btn action-btn--stat" aria-label={`${item.views} views`}>
                    <svg width="26" height="26" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                        <path d="M12 4.5C7 4.5 2.73 7.61 1 12c1.73 4.39 6 7.5 11 7.5s9.27-3.11 11-7.5c-1.73-4.39-6-7.5-11-7.5zM12 17c-2.76 0-5-2.24-5-5s2.24-5 5-5 5 2.24 5 5-2.24 5-5 5zm0-8c-1.66 0-3 1.34-3 3s1.34 3 3 3 3-1.34 3-3-1.34-3-3-3z" />
                    </svg>
                    <span className="action-count">{formatCount(item.views)}</span>
                </div>
            )}
